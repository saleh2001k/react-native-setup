...
const themeContextToTheme = (themeContext: ThemeContexts): Theme => {
  if (!themeContext || !themes[themeContext]) {
    return lightTheme
  }
  return themes[themeContext]
}
...
export const useThemeProvider = (initialTheme: ThemeContexts = undefined) => {
  ...
  const themeScheme = themeStore.selectedTheme || overrideTheme || colorScheme || "light"
  const navigationTheme = themeContextToTheme(themeScheme).isDark ? DarkTheme : DefaultTheme
...
export const useAppTheme = (): UseAppThemeValue => {
  ...
  const themeContext: ThemeContexts = useMemo(
    () => themeStore.selectedTheme || overrideTheme || (navTheme.dark ? "dark" : "light"),
    [overrideTheme, navTheme, themeStore.selectedTheme],
  )
